...
| `block-new-dependency.sh` | `PreToolUse` (Bash/Edit/Write) | **新規ライブラリ追加禁止**。`pnpm add` 等によるランタイム依存追加・`package.json` への依存追加を拒否する |
...
- `block-new-dependency.sh` は以下を許可する:
  - `pnpm add -D` / `--save-dev` / `--save-peer` / `--save-optional`(devDependencies 等。CLAUDE.md でランタイム依存の禁止対象から除外されている)
  - 引数なしの `pnpm install` / `pnpm i`(lockfile からのインストール)
